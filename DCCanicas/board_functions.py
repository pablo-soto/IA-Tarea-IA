...
## la siguiente funcion chequea los eventos de clickeo del jugador sobre el tablero y responde según corresponda
def check_events(gameStatus, screen, hole_size, board):
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if check_available(gameStatus, x, y, board, True):
...
## la siguiente función verifica si una jugada es posible de realizar
def check_available(gameStatus, x, y, board, play_move): ## el play_move guarda si efectivamente se está tratando de jugar la posición
    tile = -1
    for i, t in enumerate(board.tiles):
        if x >= t[0] and y >= t[1] and x < t[0] + t[2] and y < t[1] + t[3]:
            tile = i ## guarda la pieza donde se está tratando de jugar
            break
    if gameStatus.available[x][y] == 1:
        if play_move: print("This position has been occupied!")
        return False
    ...
    if gameStatus.tile_self != -1 and tile == gameStatus.tile_self:
        if play_move: print("The marble cannot be placed on the same tile where the player placed their previous marble. ")
        return False
    if play_move:
...
def calculate_score(gameStatus, board):
    ...
    # y cambiamos el score en el gameStatus
...
def check_end(gameStatus, board, x, y):
    for i in range(gameStatus.width):
        if check_available(gameStatus, i, y, board, False):
            return False
    for i in range(gameStatus.height):
        if check_available(gameStatus, x, i, board, False):
            return False
    return True
