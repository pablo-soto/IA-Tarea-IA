...
## la clase Board guarda las características del tablero
class Board():
    def __init__(self, screen, width, height, boardcolor, hole_size, tile_edge_color):
        ...
        self.tiles = [] ## acá se guardan las piezas rectangulares utilizadas, en formato (x,y, largox, largoy)
    ...
    def draw_available_marks(self, gameStatus, posible_marks, hole_size):
        #undreaw available marks
        for x, l in enumerate(gameStatus.available):
            for y, ava in enumerate(l):
                if ava == 0:
                    pygame.draw.circle(self.screen, (207, 169, 114), (int((x + 0.5) * hole_size), int((y + 0.5) * hole_size)), 10)

        for x,y in posible_marks:
            x = (x + 0.5) * hole_size
            y = (y + 0.5) * hole_size
            pygame.draw.circle(self.screen, (0, 254, 0), (x,y), 10)



        #draw new available marks
    ...
    def blitme(self):
        ...
        while not valid :
            ...
            for k,v in belong.items():
                if k == v:
                    tiles.append(info[k])
            print("Las piezas utilizadas son:", tiles)

            for tile in tiles: ## esta parte es para que aparezcan tableros más o menos justos
                if tile[2] * tile[3] > 6 or  tile[2] * tile[3] < 2:
                    valid = False
                    break
            
            if not BETTER_BOARDS:
                break
